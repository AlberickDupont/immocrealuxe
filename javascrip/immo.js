...
const itemWidth = container.clientWidth / slidesToShow; //largeur de l'élément HTML comprenant le padding sans border ni margin
...
btnNext.addEventListener('click',() => {
    const itemsLeft = itemsCount - (position + slidesToShow * itemWidth) / itemWidth;
...
btnPrev.addEventListener('click',() => {
    const itemsLeft = Math.abs(position)  / itemWidth //math.abs pour rester en positif.
    position += itemsLeft >= slidesToScroll ? movePosition : itemsLeft * itemWidth;
    setPosition()
    checkBtns()
});
...
const checkBtns = () => {
    btnPrev.disabled = position === 0 //valeur + type
